...
for fold in folds:
    ...
    # ---- M1-ONLY PATCH: skip Members 2-9 for this fold ----
...
# ---- M1-ONLY PATCH: dump Member-1 OOF preds keyed by (subject,item) and exit ----
import sys as _sys
from pathlib import Path as _Path
_dump_dir = _Path("/content/drive/MyDrive/prediction-competition-321M/ship/exp_loo/qwen/meta_hybrid_nbfolds")
_dump_dir.mkdir(parents=True, exist_ok=True)
assert len(primary.train) == len(p_a_train_oof), (len(primary.train), len(p_a_train_oof))
np.savez_compressed(
    _dump_dir / "m1_oof_dump.npz",
    subject_key=primary.train["subject_key"].astype(str).to_numpy(),
    item_key=primary.train["item_key"].astype(str).to_numpy(),
    bc_key=primary.train["benchmark_condition_key"].astype(str).to_numpy(),
    label=primary.train["label"].to_numpy().astype("float32"),
    fold_id=_train_row_fold_id.astype("int64"),
    p_m1_oof=p_a_train_oof.astype("float32"),
)
print("[M1-ONLY] dumped", _dump_dir / "m1_oof_dump.npz", flush=True)
_sys.exit(0)
if _M2_ENABLED:
