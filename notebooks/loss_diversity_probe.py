...
# -> two GLU layers -> linear head). The trainer + objective dispatch live
# in `src/mlp_variant.py` so the MoE POC notebook reuses the exact same
# code path (only the **loss / sample weighting** differs per experiment).
