...
# ---- M1LOO PATCH: enriched leave-one-category-out mode (env-driven) ----
import os as _os
_M1_LOO_ON = _os.environ.get("M1_LOO", "") == "1"
_M1_DROP = _os.environ.get("M1_LOO_DROP", "").strip()
if _M1_LOO_ON:
    folds = folds[:1]
    print(f"[M1LOO] single-fold mode (fold 0 only), drop variant={_M1_DROP!r}")
...
# ---- M1LOO PATCH: harness dense-shard enrichment (the categories Model A lacks) ----
_ENR_DIM = 0
if _M1_LOO_ON:
    from aide.features.cache import FeatureCache as _FC
    from aide.features.store import FoldFeatureStore as _FFS
    _DRR = "/content/drive/MyDrive/prediction-competition-321M"
    _enr_store = _FFS(_FC(f"{_DRR}/features", code_version="v2"),
                      embedding_family="qwen", seed=0, n_folds=3)
    _GEOMG = ["centroid_distance", "cluster_geometry", "nn_geometry", "item_cluster"]
    _LABG = ["nn_label_derivatives", "cluster_passrate", "cluster_subject",
             "counts_subject"]
    _gb, _enr_geo_index = [], None
    _slices, _off = {}, 0
    for _g in _GEOMG:
        _sh = _enr_store.cache.read_shard(_enr_store._key(_g, "all"))
        if _enr_geo_index is None:
            _enr_geo_index = {str(k): i for i, k in enumerate(_sh.row_ids)}
        _X = np.asarray(_sh.X, np.float32)
        _slices[_g] = (_off, _off + _X.shape[1]); _off += _X.shape[1]
        _gb.append(_X)
    _enr_geo = np.concatenate(_gb, axis=1); del _gb
    _GEO_DIM = _enr_geo.shape[1]
    _lb_parts, _lb_rids = [], []
    for _f in range(3):
        _bk = []
        _rids_f = None
        for _g in _LABG:
            ...
            if _f == 0:
                _slices[_g] = (_off, _off + _X.shape[1]); _off += _X.shape[1]
            _bk.append(_X)
        _lb_parts.append(np.concatenate(_bk, axis=1)); _lb_rids.extend(_rids_f)
    _enr_lab = np.concatenate(_lb_parts, axis=0); del _lb_parts
    _enr_lab_index = {r: i for i, r in enumerate(_lb_rids)}; del _lb_rids
    _ENR_DIM = _GEO_DIM + _enr_lab.shape[1]
    _ENR_MU = np.concatenate([_enr_geo.mean(0), _enr_lab.mean(0)])
    _ENR_SD = np.clip(np.concatenate([_enr_geo.std(0), _enr_lab.std(0)]), 1e-6, None)
    _ENR_CAT_SLICES = {
        "geometry": (0, _GEO_DIM),
        "nn_label_derivatives": _slices["nn_label_derivatives"],
        "subject_label_aggs": (_slices["cluster_passrate"][0],
                               _slices["counts_subject"][1]),
    }
    print(f"[M1LOO] enrichment ready: {_ENR_DIM} cols, geo={_GEO_DIM}, "
          f"slices={_ENR_CAT_SLICES}")


def _enrich_matrix(split_part):
    iks = split_part["item_key"].astype(str).to_numpy()
    sks = split_part["subject_key"].astype(str).to_numpy()
    out = np.zeros((len(iks), _ENR_DIM), np.float32)
    gi = np.fromiter((_enr_geo_index.get(k, -1) for k in iks), np.int64, len(iks))
    ok = gi >= 0
    out[ok, :_GEO_DIM] = _enr_geo[gi[ok]]
    ri = np.fromiter((_enr_lab_index.get(s + "|" + i, -1)
                      for s, i in zip(sks, iks)), np.int64, len(iks))
    ok2 = ri >= 0
    out[ok2, _GEO_DIM:] = _enr_lab[ri[ok2]]
    out -= _ENR_MU[None, :]
    out /= _ENR_SD[None, :]
    if _M1_DROP in _ENR_CAT_SLICES:
        lo, hi = _ENR_CAT_SLICES[_M1_DROP]
        out[:, lo:hi] = 0.0
    return out
...
def _build(split_part, nn_mat):
    ...
    if _M1_LOO_ON:
        pf = np.concatenate([np.asarray(pf, np.float32),
                             _enrich_matrix(split_part)], axis=1)
        if _M1_DROP == "nn15":
            nn_mat = np.zeros_like(nn_mat)
...
_common_kwargs = dict(
    ...
    pool_feature_dim=len(POOL_FEATURE_NAMES_EXT) + _ENR_DIM,
...
# ---- M1LOO PATCH: channel-level LOO variants ----
if _M1_LOO_ON and _M1_DROP == "metadata":
    model_a_cfg = ModelConfig(use_metadata_features=False, **_common_kwargs)
elif _M1_LOO_ON and _M1_DROP == "cluster":
    _ck = dict(_common_kwargs)
    _ck["use_cluster_features"] = False
    _ck["cluster_embed_dim"] = 0
    model_a_cfg = ModelConfig(
        use_metadata_features=True,
        meta_subject_categorical=_meta_schema.subject_categorical,
        meta_subject_numeric=_meta_schema.subject_numeric,
...
for fold in folds:
    ...
    if OOF_RETRAIN_M1:
        ...
        _fold_m1_bundle = cache_or_compute(
            ...
            key_inputs=(
                ...
                "m1loo:" + _M1_DROP if _M1_LOO_ON else "",
                fold_suffix,
                json.dumps(asdict(model_a_cfg), default=str, sort_keys=True),
                json.dumps(asdict(train_cfg), default=str, sort_keys=True),
                json.dumps(asdict(a_drop), default=str, sort_keys=True),
    ...
    # ---- M1LOO PATCH: dump this variant's fold-0 OOF preds and exit ----
    if _M1_LOO_ON:
        import sys as _sys
        from pathlib import Path as _Path
        _vd = _Path("/content/drive/MyDrive/prediction-competition-321M/ship/exp_loo/"
                    "qwen/meta_hybrid_loo")
        _vd.mkdir(parents=True, exist_ok=True)
        _tag = _M1_DROP if _M1_DROP else "FULL"
        _yy = primary.train["label"].to_numpy().astype("float32")[fold.oof_row_idx]
        np.savez_compressed(
            _vd / f"fold0_{_tag}.npz",
            subject_key=primary.train["subject_key"].astype(str).to_numpy()[fold.oof_row_idx],
            item_key=primary.train["item_key"].astype(str).to_numpy()[fold.oof_row_idx],
            label=_yy,
            p=p_a_oof_fold.astype("float32"),
        )
        _pp = np.clip(p_a_oof_fold.astype("float64"), 1e-7, 1 - 1e-7)
        _bce = float(-(_yy * np.log(_pp) + (1 - _yy) * np.log(1 - _pp)).mean())
        print(f"[M1LOO] variant={_tag} fold0 n={len(_yy)} soft_bce={_bce:.6f}", flush=True)
        _sys.exit(0)
    print(f"[M1-ONLY] fold {fold.fold_id} M1 OOF written; skipping members 2-9")
    continue
