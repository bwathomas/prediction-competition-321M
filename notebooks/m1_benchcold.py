...
# ---- BENCHCOLD PATCH: replace item folds with ONE benchmark-holdout fold ----
import os as _os
_M1_BC = _os.environ.get("M1_HOLDOUT_BENCH", "").strip()
if _M1_BC:
    from src.oof_folds import ItemFold as _ItemFold
    _bench_rows = primary.train["benchmark"].astype(str).to_numpy()
    _bc_oof = np.where(_bench_rows == _M1_BC)[0].astype(np.int64)
    _bc_tr = np.where(_bench_rows != _M1_BC)[0].astype(np.int64)
    if len(_bc_oof) == 0:
        raise ValueError(f"M1_HOLDOUT_BENCH {_M1_BC!r} has 0 rows")
    _ik = primary.train["item_key"].astype(str).to_numpy()
    folds = [_ItemFold(
        fold_id=0,
        train_item_keys=tuple(np.unique(_ik[_bc_tr]).tolist()),
        oof_item_keys=tuple(np.unique(_ik[_bc_oof]).tolist()),
        train_row_idx=_bc_tr,
        oof_row_idx=_bc_oof,
    )]
    print(f"[BENCHCOLD] holdout benchmark={_M1_BC!r}: train_rows={len(_bc_tr):,} "
          f"oof_rows={len(_bc_oof):,} -- folds REPLACED with 1 bench-holdout fold")
...
for fold in folds:
    ...
    # ---- M1-ONLY / BENCHCOLD PATCH ----
    if _M1_BC:
        import sys as _sys
        from pathlib import Path as _Path
        _dd = _Path("/content/drive/MyDrive/prediction-competition-321M/ship/exp_cold/"
                    f"meta_hybrid_benchcold/{_M1_BC}")
        _dd.mkdir(parents=True, exist_ok=True)
        _yy = primary.train["label"].to_numpy().astype("float32")[fold.oof_row_idx]
        np.savez_compressed(
            _dd / "benchcold_preds.npz",
...
import sys as _sys
print("[M1-ONLY] all folds done (no benchcold env set); exiting without stacker")
_sys.exit(0)
if _M2_ENABLED:
