...
def _verify_one(m: str, labeling_py: str, meta: str) -> None:
    ...
    assert "_RIDGE_LAMBDA" in m
    assert "n_repeats" in m
    assert "self.per_bc" in m
    assert "self.per_bc: dict[str, dict]" in m
    assert "margin_nats_per_param" in m
    assert "require_majority_repeat_wins" in m
